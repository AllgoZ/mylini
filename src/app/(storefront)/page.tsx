            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
              {bestSellers.map((product, i) => (
                <ProductCard
                  key={product.id}
                  product={product}
                  emoji={emojiSet1[i % emojiSet1.length]}
                  gradient={gradientSet1[i % gradientSet1.length]}
                  delay={0.05 * i}
                  priority={i < 2}
                />
              ))}
            ...
            {promoBlocks.map((block, i) => (
              ...
              >
                ...
                <div className="relative z-10 p-7 md:p-8">
                  {block.body_text && (
                    <div className="text-[0.68rem] font-bold tracking-[0.14em] uppercase text-white/80 mb-2">{block.body_text}</div>
                  )}
                  {block.title && (
                    <h3 className="font-head text-[1.6rem] font-bold text-white leading-[1.2] mb-1.5 drop-shadow-sm">
                      {block.title.includes(' ') ? (
                        <>
                          ...
                          <em className="italic text-gold-pale">{block.title.split(' ').at(-1)}</em>
                        </>
                      ) : (
                        block.title
                      )}
                  ...
                  {block.subtitle && (
                    <p className="text-[0.85rem] text-white/90 mb-4 drop-shadow-sm">{block.subtitle}</p>
                  )}
                  <div className="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-md border border-white/40 text-white text-[0.8rem] font-bold py-2 px-4.5 rounded-full transition-colors group-hover:bg-white/30">
                    {block.link_text ?? 'Explore'} <ChevronRight size={14} />
                  </div>
                </div>
              </Link>
            ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4">
              {newArrivals.map((product, i) => (
                <ProductCard
                  key={product.id}
                  product={product}
                  emoji={emojiSet2[i % emojiSet2.length]}
                  gradient={gradientSet2[i % gradientSet2.length]}
                  delay={0.05 * i}
                  priority={i < 2}
                />
              ))}
